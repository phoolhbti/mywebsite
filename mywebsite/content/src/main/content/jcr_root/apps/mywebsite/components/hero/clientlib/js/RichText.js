 ...
 *     {@link CQ.form.rte.plugins.FormatPlugin "format" plugin}</li>
 *   <li>Paragraph/block alignment (left, right, center) - see
 *     {@link CQ.form.rte.plugins.JustifyPlugin "justify" plugin}</li>
 *   <li>Hyperlinks and anchors - see {@link CQ.form.rte.plugins.LinkPlugin "links" plugin}
 *     </li>
 *   <li>List implementation (including support for nested lists) - see
 *     {@link CQ.form.rte.plugins.ListPlugin "lists" plugin}</li>
 *   <li>Indent/outdent of blocks/paragraphs - see
 *     {@link CQ.form.rte.plugins.ListPlugin "lists" plugin}</li>
 *   <li>Applying formats (p, h1, h2, h3, ...) to a paragraph - see
 *     {@link CQ.form.rte.plugins.ParagraphFormatPlugin "paraformat" plugin}</li>
 *   <li>Applying styles to text fragments (using CSS classes) - see
 *     {@link CQ.form.rte.plugins.StylesPlugin "styles" plugin}</li>
 *   <li>Table support (basic support as of 5.2, full support as of 5.3)- see
 *     {@link CQ.form.rte.plugins.TablePlugin "table" plugin}</li>
 *   <li>Advanced support for pasting text (plain text and cleaned HTML/Word) - see
 *     {@link CQ.form.rte.plugins.EditToolsPlugin "edit" plugin}</li>
 *   <li>HTML source editing - see
 *     {@link CQ.form.rte.plugins.MiscToolsPlugin "misctools" plugin}</li>
 *   <li>Find &amp; Replace - see
 *     {@link CQ.form.rte.plugins.FindReplacePlugin "findreplace" plugin}</li>
...
[
    ...
    /**
     * The {@link CQ.form.rte.EditorKernel} to be used by this RichText instance
     * @type CQ.form.rte.IFrameKernel
     * @private
     */
    editorKernel: null,
    ...
    constructor: function(config) {
        ...
        var defaults = {
            ...
            "height": CQ.themes.RichText.DEFAULT_HEIGHT,
            "requiredCssPath": CQ.themes.RichText.DEFAULT_REQCSS_PATH,
    ...
    onResize: function(w, h) {
        ...
        if (this.el && this.iframe) {
            ...
            } else if ((typeof(w) == "string") && CQ.form.rte.Common.strEndsWith(w, "%")) {
            ...
            var tb = this.editorKernel.tb;
            if (tb) {
                var tbCnt = tb.length;
                for (tbIndex = 0; tbIndex < tbCnt; tbIndex++) {
                    tb[tbIndex].onResize(w, tb[tbIndex].el.getHeight());
                }
            }
    ...
    createEditContext: function() {
        if (this.initialized) {
            return new CQ.form.rte.EditContext(this.iframe, this.getWin(), this.getDoc(),
                    this.getEditorBody());
        }
            ...
            if (styleDefs != null) {
                ...
                for (var t = 0; t < typeCnt; t++) {
                    ...
                    for (var s = 0; s < styleCnt; s++) {
                        ...
                        if (!CQ.form.rte.Common.arrayContains(cssName)) {
    ...
    onEditorEvent: function(e) {
        // nothing to do here -> moved to editor kernel
        // todo do we really need to still override this?
    },
...
<pre>
{
     ...
     * <p>Note that the {@link CQ.form.rte.plugins.StylesPlugin "styles" plugin} must be
     ...
     * {@link CQ.form.rte.LinkPlugin "links" plugin} for this configuration to take effect.
     * Defaults to:</p>
<pre>
{
    "protocols": [
     ...
     * been enabled through the {@link CQ.form.rte.plugins.LinkPlugin "links" plugin} for
